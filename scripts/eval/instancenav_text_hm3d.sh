CUDA_VISIBLE_DEVICES=0 MAGNUM_LOG=quiet HABITAT_SIM_LOG=quiet python run.py \
    ...
    --run-type eval --model-dir exps/eval/PSL-instancenav-text \
    ...
    TASK_CONFIG.DATASET.DATA_PATH data/datasets/instancenav/val/val_text.json.gz \
    TASK_CONFIG.DATASET.TYPE AttrObjNav-v1 \
    ...
    TASK_CONFIG.TASK.QUERIED_IMAGE_GOAL_SENSOR.DATA_PATH data/goal_datasets/instancenav/val.queried_image_goal.imagenav_v2.pth \
